...
restaurants = transform_to_df_join(
    db,
    "SELECT * FROM restaurants, geographie WHERE restaurants.id_restaurant = geographie.id_restaurant;",
)

avis_requete = transform_to_df_join(
    db,
    f"""SELECT avis.date_avis, avis.titre_avis, avis.contenu_avis, avis.note_restaurant, avis.label, restaurants.nom 
...
with tab1:
    ...
    moyenne_par_an = retrieve_year(
        avis, "avis.date_avis", "année", "avis.note_restaurant", "mean"
    )
...
with tab2:
    ...
    selected_year = st.selectbox(
        "Sélectionnez une année pour voir les données par mois", moyenne_par_an["année"]
    )
    ...
    moyenne_par_mois = (
        avis_par_an.groupby("mois_nom")["avis.note_restaurant"].mean().reset_index()
    )
    ...
    moyenne_par_mois["mois_nom"] = pd.Categorical(
        moyenne_par_mois["mois_nom"], categories=mois_ordre, ordered=True
    )
...
with tab1:
    ...
    avis["avis.label_sentiment"] = avis["avis.label"].replace(
        {5: "Positif", 4: "Positif", 3: "Neutre", 2: "Négatif", 1: "Négatif"}
    )

    # Calcul des proportions de chaque sentiment
    total_avis = len(avis)
    positif = (
        avis["avis.label_sentiment"].value_counts().get("Positif", 0) / total_avis * 100
    )
    neutre = (
        avis["avis.label_sentiment"].value_counts().get("Neutre", 0) / total_avis * 100
    )
    negatif = (
        avis["avis.label_sentiment"].value_counts().get("Négatif", 0) / total_avis * 100
    )
    ...
    with col9:
        st.markdown(
            markdown_style.format("green", f"Proportion de positif : {positif:.2f}%"),
            unsafe_allow_html=True,
        )
    with col10:
        st.markdown(
            markdown_style.format("orange", f"Proportion de neutre : {neutre:.2f}%"),
            unsafe_allow_html=True,
        )
    with col11:
        st.markdown(
            markdown_style.format("red", f"Proportion de négatif : {negatif:.2f}%"),
            unsafe_allow_html=True,
        )

    # Calculer les proportions de chaque sentiment par année
    sentiment_counts = (
        avis.groupby(["année", "avis.label_sentiment"]).size().unstack(fill_value=0)
    )
...
with tab2:
    st.markdown(
        """La note de sentiment est une note attribuée par un modèle à chaque avis. Elle est comprise entre 1 et 5, 1 étant très négatif et 5 très positif. <br>
    ...
    fig_distribution.add_trace(
        go.Histogram(
            x=avis["avis.label"], name="Prédictions", marker_color="blue", opacity=0.7
        )
    )
...
if st.session_state["show_avis"]:
    ...
    with col3:
        debut_date = st.date_input(
            "Choissiez votre date de début", min_date, label_visibility="visible"
        )

    with col4:
        fin_date = st.date_input(
            "Choissiez votre date de fin", max_date, label_visibility="visible"
        )
    ...
    with col12:
        ...
        selected_sentiments = st.multiselect(
            "Sélectionnez les sentiments qui vous intéressent", unique_sentiments
        )
    ...
    if selected_notes:
        filtered_avis = filtered_avis[
            filtered_avis["Note du restaurant"].isin(selected_notes)
        ]

    if selected_sentiments:
        filtered_avis = filtered_avis[
            filtered_avis["Sentiment"].isin(selected_sentiments)
        ]

    if search_text:
        filtered_avis = filtered_avis[
            filtered_avis["Contenu de l'avis"].str.contains(
                search_text, case=False, na=False
            )
        ]

    # Formater les dates pour l'affichage
    filtered_avis.loc[:, "Date de l'avis"] = pd.to_datetime(
        filtered_avis["Date de l'avis"]
    ).dt.strftime("%Y/%m/%d")
    ...
    for index, row in filtered_avis.iterrows():
        ...
        with st.expander(
            f"Avis du {date_avis} - Note : {stars} - Sentiment : {sentiment}",
            expanded=True,
        ):
